...
### 如何启动
- 源码直接运行WtkTakehomeApplication中的main方法

### AI 部分的设计思路
- 初始化一个本地ai client，并预设角色为“酒店客服，需要总结用户评论内容”
- 编写一句话摘要ai功能对应的工具方法，描述关键词提及“对用户评论的一句话摘要”内容，对应实现为查询具体用户评论功能

### 如果有更多时间会怎么改进
- 新增评论补充参数校验
- 考虑使用分布式锁方式实现幂等，防止重复提交
- ai功能可以预设一个向量化数据库，把所有酒店的所有用户评论放入这个向量数据库中，后续用户在查询具体酒店时，通过ai对话可以直接了解酒店的整体评价情况
- ai端点返回数据的方式可以使用Flux流式，减少用户等待时间，提升用户体验
